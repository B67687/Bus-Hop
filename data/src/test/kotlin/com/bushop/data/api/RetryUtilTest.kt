...
class RetryUtilTest {
    ...
    @Test
    fun `success after retries returns result`() = runTest {
        var attempts = 0
        val result =
            retrySuspend(maxRetries = 2) {
                attempts++
    ...
    @Test
    fun `failure after all retries propagates last error`() = runTest {
        val result =
    ...
    @Test
    fun `cancellation exception is rethrown not retried`() = runTest {
        var attempts = 0
        try {
    ...
    @Test
    fun `exception thrown from block is caught and retried`() = runTest {
        var attempts = 0
        val result =
    ...
    @Test
    fun `retry respects maxRetries zero`() = runTest {
        var attempts = 0
        val result =
