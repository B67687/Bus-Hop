...
class StopRefreshCoordinatorTest {
    ...
    @Test
    fun `first refresh is always allowed`() = runTest {
        assertTrue(coordinator.tryRefresh("12345", isAutoRefresh = false))
    }

    @Test
    fun `auto refresh within cooldown is allowed`() = runTest {
        assertTrue(coordinator.tryRefresh("12345", isAutoRefresh = false))
    ...
    @Test
    fun `manual refresh within cooldown is blocked`() = runTest {
        assertTrue(coordinator.tryRefresh("12345", isAutoRefresh = false))
        assertFalse(coordinator.tryRefresh("12345", isAutoRefresh = false))
    }

    @Test
    fun `different stops have independent cooldowns`() = runTest {
    ...
    @Test
    fun `refreshAllConcurrent refreshes all codes`() = runTest {
    ...
    @Test
    fun `chunking limits concurrent refreshes`() = runTest {
