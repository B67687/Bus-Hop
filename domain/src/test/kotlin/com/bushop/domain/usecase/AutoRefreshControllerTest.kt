...
class AutoRefreshControllerTest {
    @Test
    fun `start does not immediately invoke callback`() = runTest {
    ...
    @Test
    fun `callback fires after real delay elapses`() = runTest {
        val controller = AutoRefreshController(this)
        var count = 0
    ...
    @Test
    fun `zero interval does not start`() = runTest {
        val controller = AutoRefreshController(this)
        var invoked = false
    ...
    @Test
    fun `negative interval does not start`() = runTest {
        val controller = AutoRefreshController(this)
        var invoked = false
        controller.start(intervalSeconds = -1) { invoked = true }
        controller.stop()
    ...
    @Test
    fun `restart cancels previous interval`() = runTest {
        val controller = AutoRefreshController(this)
        var count = 0
        controller.start(intervalSeconds = 1) { count++ }
    ...
    @Test
    fun `stop stops the controller`() = runTest {
        val controller = AutoRefreshController(this)
        var count = 0
        controller.start(intervalSeconds = 1) { count++ }
        val afterFirst = count
